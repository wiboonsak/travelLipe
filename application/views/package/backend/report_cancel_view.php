...
<hr>
    ...
    <form method="post" action="<?php echo base_url(); ?>PackageCMS/action2">
...
<br>
                            <table id="table2" class="table table-hover">
                                ...
                                <tbody>
 <?php 
  $checkinData =$this->Package_model->Reportcancel();
    foreach ($checkinData->result() AS $Data) {?>	
                                        <tr id="row<?php echo $Data->id ?>" class="removech">
                                            ...
                                            <td><?php if ($Data->cf_status == 1){echo 'Pending Payment 
Credit card';}else if($Data->cf_status == 2){echo 'Confrim Payment Credit card';}else{echo 'Cancel';} ?></td>
                                            <td style="text-align:center;"><?php echo $this->Package_model->GetEngDateTime($Data->date_booking);?></td>
                                            <td style="text-align:center;" > <a href="#" onClick="windowOpener('770', '1000', 'windowName', 'email_book_package/<?php echo $Data->transfer_keygroup?>')"><button type="button" class="btn btn-xs btn-info" data-toggle="button" style="width: 88.28px" >Detail</button></a></td>
                                        </tr>
                                    <?php }?>
                                </tbody>
                            </table>
